...
int pc_checkjoblevelup(struct map_session_data *sd)
{
	...
	if (pc_checkskill(sd, SG_DEVIL) && sd->status.job_level >= MIN_DEVIL_LEVEL)
...
short pc_maxparameter(struct map_session_data *sd, enum e_params param) {
	...
	return ((class_&MAPID_UPPERMASK) == MAPID_KAGEROUOBORO || (class_&MAPID_UPPERMASK) == MAPID_REBELLION || (class_&MAPID_UPPERMASK) == MAPID_STAR_GLADIATOR || (class_&MAPID_UPPERMASK) == MAPID_SOUL_LINKER) ? battle_config.max_extended_parameter :
